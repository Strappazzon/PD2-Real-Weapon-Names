...
Hooks:PostHook(
	BlackMarketGui,
	"update_info_text",
	"RWN_BlackMarketGuiPostUpdateInfoText",
	function(self)
		local tab_data = self._tabs[self._selected]._data
		local identifier = tab_data.identifier

		local is_renaming_this =
			self._renaming_item and not self._data.is_loadout and self._renaming_item.category == self._slot_data.category and self._renaming_item.slot == self._slot_data.slot
...
Hooks:PostHook(
	BlackMarketGui,
	"mouse_moved",
	"RWN_BlackMarketGuiPostMouseMoved",
	function(self, o, x, y)
...
Hooks:PostHook(
	...
	"_init_melee_weapons",
	"RWN_BlackMarketTweakDataPostInitMeleeWeapons",
